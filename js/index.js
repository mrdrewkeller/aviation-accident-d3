...
var accidents = "http://174.138.68.14:3000/accidents?amateurbuilt=eq.Yes&publicationdate=gt.2016-04-04 00:00:00";
request = new XMLHttpRequest();
request.open('GET', accidents, true);
request.onload = function () {
  accidentLocations = JSON.parse(request.responseText);
};
request.send();
...
d3.json("https://raw.githubusercontent.com/moigithub/d3maplayout/master/world-50m.json", function (error, data) {
  ...
  d3.json(accidents, function (error, data) {
    ...
    // Render the circles
    points.append("circle").attr("r", .3).attr("cx", function (d) {
      ...
      .on("mouseover", function (d) {
        ...
        // Add text to .tooltip
        tooltip.html("<h2 class='title'>" + data.location + "</h2>" + "<h3>Fell: " + data.country + "</h3>" + "<strong>Mass</strong>: " + (data.mass / 1000).toFixed(2) + " KG<br />");
